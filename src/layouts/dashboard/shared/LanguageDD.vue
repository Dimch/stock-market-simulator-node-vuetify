...
const languageOptions = shallowRef([
...
<template>
  ...
  <v-list class="py-0" aria-label="language list" aria-busy="true">
    <v-list-item v-for="(item, index) in languageOptions" :key="index" color="primary" class="d-flex align-center"
                 :active="$i18n.locale === item.value" @click="() => ($i18n.locale = item.value)">
